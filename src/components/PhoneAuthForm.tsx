...
export default function PhoneAuthForm({ onSendCode, onVerifyCode }: Props) {
  ...
  return (
    <div className="flex min-h-0 flex-1 flex-col">
      ...
      {step === "phone" ? (
        <form onSubmit={handleSendCode} className="space-y-3">
          <input
            ...
            className="w-full rounded-lg border border-white/10 bg-white/5 px-3 py-2.5 text-sm text-white placeholder:text-white/30 outline-none focus:border-[#FF2D8A]/50"
            autoComplete="tel"
          />
          <button
            type="submit"
            disabled={loading || !phone.trim()}
            className="w-full rounded-lg bg-[#FF2D8A] py-2.5 text-sm font-semibold text-[#07060B] transition hover:brightness-110 disabled:opacity-50"
          >
      ...
      {error && <p className="mt-3 text-sm text-[#FF2D8A]">{error}</p>}
